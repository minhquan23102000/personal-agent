...
class ContextMemoryUpdate(BaseModel):
    ...
    agent_background: str = Field(
        description="Update your profile to reflect changes in name, gender, language, style, age, role, personality, identity, historical background, and other relevant aspects based on the conversation. Ensure the profile is descriptive, detailed, and free of any instructions or additional information."
    )
...
def context_memory_prompt(history, current_memory, user_feedback): ...


async def context_memory_update(
    ...
    try:
        ...
        @litellm.call(
            ...
            response_model=ContextMemoryUpdate,
            json_mode=True,
        )
